...
void NativeRender::InitRender(JNIEnv *env, int video_width, int video_height, int *dst_size) {
    //初始化窗口
    m_native_window = ANativeWindow_fromSurface(env, m_surface_ref);//将Surface绑定给本地窗口
    //绘制区域的宽高，获取Surface可显示区域的宽高
    ...
    //设置宽高,限制缓冲区中的像素数量。这是本地窗口缓冲区大小，完成初始化
